...
def load_and_evaluate():
    ...
    # ── Fraud ──────────────────────────────────────────────────────
    fraud_model = joblib.load(r"Fraud_Detection_Model/model/model.pkl")
    ...
    yf_pred, yf_proba  = make_prediction(Xft)
    yf_proba = yf_proba[:, 1]

    # ── Credit ─────────────────────────────────────────────────────
    credit_model = joblib.load(r"Credit_risk_model/saved_models/credit_risk_model.pkl")
...
with tab1:
    ...
    with r1:
        ...
        ax.set_title("Fraud Confusion Matrix")
        plt.tight_layout()
    ...
    with r2:
        ...
        roc_auc = auc(fpr, tpr)
        ax.plot(fpr, tpr, color='#1a73e8', lw=2, label=f'AUC = {roc_auc:.4f}')
        ax.plot([0,1],[0,1],'--', color='gray', lw=1)
        ax.set_xlabel("False Positive Rate"); ax.set_ylabel("True Positive Rate")
        ax.set_title("ROC Curve — Fraud Model")
        ax.legend(loc='lower right')
        plt.tight_layout()
        st.pyplot(fig); plt.close()

    st.markdown("**Top 10 Feature Importances**")
    preprocessor = joblib.load(r"Fraud_Detection_Model/preprocessor.pkl")
    classifier   = fraud_model
    feature_names = raw_features = preprocessor.get_feature_names_out()
    ...
    ax.set_title("Top 10 Feature Importances — Fraud", fontweight='bold')
...
with tab2:
    ...
    with r2:
        ...
        roc_auc = auc(fpr, tpr)
        ax.plot(fpr, tpr, color='#ea4335', lw=2, label=f'AUC = {roc_auc:.4f}')
        ax.plot([0,1],[0,1],'--', color='gray', lw=1)
        ax.set_xlabel("False Positive Rate"); ax.set_ylabel("True Positive Rate")
    ...
    st.markdown("**Top 10 Feature Importances**")
    preprocessor = credit_model.named_steps['preprocessor']
    classifier   = credit_model.named_steps['classifier']
    ...
    ax.set_title("Top 10 Feature Importances — Credit Risk", fontweight='bold')
    ax.set_xlabel("Importance Score")
    plt.tight_layout()
    st.pyplot(fig); plt.close()
